...
func (m *MockRedisStore) AddNX(ctx context.Context, key string, value string, exp time.Duration) (bool, error) {
...
func (m *MockRedisStore) Que(ctx context.Context, mode string, region string, player *models.Player) error {
	args := m.Called(ctx, mode, region, player)
	return args.Error(0)
}

func (m *MockRedisStore) DeQue(ctx context.Context, mode string, region string, count int) ([]*models.Player, error) {
	args := m.Called(ctx, mode, region, count)
	players, _ := args.Get(0).([]*models.Player)
	return players, args.Error(1)
}

func (m *MockRedisStore) DeQuePlayer(ctx context.Context, mode string, region string, playerID string) error {
